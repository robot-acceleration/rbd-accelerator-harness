...
#-------------------------------------------------------------------------------
# dqd Forward Pass (Link i, Input j) Control Unit
#-------------------------------------------------------------------------------
function dqdFPij(CUSTOM_TYPE,X,Y,Z,AX,AY,AZ,LX,LY,LZ,
                tcurrXprev,lcurr_I,
   ...
   s1reg1,s1reg2,s1reg3,s1reg4 = lcurr_v,dadqd_xdot_vec,lcurr_qd,dvdqd_xdot_vec
   s1r1_in,s1r2_in,s1r3_in,s1r4_in = s1reg1,s1reg2,s1reg3,s1reg4
   # stage 1 computation
   s1r1_out,s1r2_out,s1r3_out,s1r4_out,s1r5_out,
   s2r1_out,s2r2_out,s2r3_out,s2r4_out,s2r5_out,
   s3r1_out,s3r2_out,s3r3_out = dqdFPij_folded(CUSTOM_TYPE,X,Y,Z,AX,AY,AZ,LX,LY,LZ,
   ...
   # assign outputs
   lcurr_dfi_dqdj,lcurr_dai_dqdj,lcurr_dvi_dqdj = s3r1_out,s3r2_out,s3r3_out
...
#-------------------------------------------------------------------------------
# dqd Forward Pass (Link i, Input j) Folded
#-------------------------------------------------------------------------------
function dqdFPij_folded(CUSTOM_TYPE,X,Y,Z,AX,AY,AZ,LX,LY,LZ,
                       tcurrXprev,lcurr_I,mcross,
                       s1_bool,s2_bool,s3_bool,
                       s1r1_in,s1r2_in,s1r3_in,s1r4_in,
                       s2r1_in,s2r2_in,s2r3_in,s2r4_in,s2r5_in,
   ...
   # mx1
   mx1_out   = zeros(CUSTOM_TYPE,6,1)
   # mxv
   mxv_out   = zeros(CUSTOM_TYPE,6,1)
   ...
   # (1 add)
   if mcross
      mx1_out[AZ] = xdot_out[AZ] + 1.0;
   else
      mx1_out[AZ] = xdot_out[AZ]
   end
   mx1_out[AX] = xdot_out[AX]
   mx1_out[AY] = xdot_out[AY]
   mx1_out[LX] = xdot_out[LX]
   mx1_out[LY] = xdot_out[LY]
   mx1_out[LZ] = xdot_out[LZ]
   # mxv
   # (4 adds)
   if mcross
      mxv_out[AX] = add2_out[AX] +  s2r1_in[AY]
      mxv_out[AY] = add2_out[AY] + -s2r1_in[AX]
      mxv_out[LX] = add2_out[LX] +  s2r1_in[LY]
      mxv_out[LY] = add2_out[LY] + -s2r1_in[LX]
   else
      mxv_out[AX] = add2_out[AX]
      mxv_out[AY] = add2_out[AY]
      mxv_out[LX] = add2_out[LX]
      mxv_out[LY] = add2_out[LY]
   end
   mxv_out[AZ] = add2_out[AZ]
   mxv_out[LZ] = add2_out[LZ]
